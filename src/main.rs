...
fn print_help() {
    ...
    println!();
    println!("Options:");
    println!("  -w, --words <exact|min-max>   Generate text with exact words or range of words (e.g., 500 or 100-500)");
    println!("  -b, --bytes <exact|min-max>   Generate text with exact bytes or range of bytes (e.g., 1000 or 500-1500)");
...
fn main() {
    ...
    // Helper closure to sample a random word based on frequencies
    let sample_word = |rng: &mut ChaCha8Rng| -> &str {
        let r = rng.gen_range(1..=sum);
        let idx = match cdf.binary_search(&r) {
            Ok(i) => i,
            Err(i) => i,
        };
        words[idx]
    ...
    if let Some(bytes_range) = cli_bytes {
        ...
        while bytes_generated < target_bytes {
            ...
            for _ in 0..paragraph_sentences_count {
                ...
                for i in 0..sentence_len {
                    let mut word = sample_word(&mut rng).to_string();
                    if i == 0 {
                ...
                let added_len = sentence_str.len() + if sentences.is_empty() { 0 } else { 1 };
            ...
            if !sentences.is_empty() {
                ...
                if bytes_written + chunk.len() <= target_bytes {
                    writer.write_all(chunk.as_bytes()).expect("Failed to write to stdout");
                    ...
                    writer.write_all(&chunk.as_bytes()[..end]).expect("Failed to write to stdout");
                    break;
                }
            }
        }
    } else {
        let target_words = cli_words
        ...
        while words_generated < target_words {
            ...
            if !sentences.is_empty() {
                ...
                writer.write_all(sentences.join(" ").as_bytes()).expect("Failed to write to stdout");
            }
        }
    }
